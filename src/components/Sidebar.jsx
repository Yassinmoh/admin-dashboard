                ...
                (<>
                    <div className="flex justify-between items-center">
                        ...
                        <TooltipComponent content="Menu" position='BottomCenter'>
                            <button type="button" className="text-xl rounded-full p-3 hover:bg-light-gray mt-4 block " onClick={() =>setActiveMenu((prevActiveMenu)=>!prevActiveMenu)}> {/* md:hidden*/}
                                <MdOutlineCancel />
                            </button>
                        </TooltipComponent>
                    </div>
                    <div className="mt-10">
                        {links.map((item)=>(
                            <div key={item.title}>
                                ...
                                {item.links.map((link)=>(
                                    <NavLink to={`/${link.name}`} key={Math.random()} onClick={handleCloseSidebar} className={({isActive})=>
